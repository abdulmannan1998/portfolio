...
7. Verify TechAndCodeSection passes commits prop to GitHubActivity: `grep "commits={commits}" components/sections/tech-and-code-section.tsx` matches.
8. Run `npm run dev`, visit the page. View source (Ctrl+U) and confirm the GitHub commits HTML is present in the initial server response -- this proves zero-flash.
   </verify>
