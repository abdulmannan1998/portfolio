...
must_haves:
  truths:
    ...
    - "Hero section content is visible from SSR through hydration with zero flash, then plays subtle settle animations (scale pulse, bar extend, subtitle rise)"
    - "No perceptible flash of invisible content during hydration transition for any section"
  artifacts:
    - path: "lib/use-hydrated.ts"
      provides: "Hydration detection hook"
      exports: ["useHydrated"]
    - path: "components/sections/hero-section.tsx"
      provides: "Hero with initial={false} + settle keyframe animations (above-fold, no key remount)"
      contains: "initial={false}"
...
<objective>
Fix GAP-1: PPR static shell is visually empty because framer-motion sets inline `style="opacity: 0"` on 13 motion elements during server rendering. Content must be visible in the prerendered HTML while preserving entrance animations after hydration.
...
### Two strategies: above-fold vs below-fold

**Above-fold (hero):** Use `initial={false}` (no SSR inline styles) + subtle "settle" keyframe animations that start from the visible state. Content is always visible — the animation enhances rather than reveals. No `key` remount needed, no `useHydrated` needed. The `animate` prop changes from a static end-state to keyframe arrays that start from the current visual appearance.

**Below-fold (all other sections):** Use `initial={isHydrated ? {...} : false}` + `key` remount. These sections use `whileInView` and haven't been scrolled to when hydration occurs, so the remount flash is invisible to the user. Entrance animations play normally when scrolled into view.

### Specific changes per component:

**hero-section.tsx** (4 elements) — ABOVE-FOLD, settle animations:

- `motion.h1`:
  - Change `initial={{ y: 100, opacity: 0 }}` to `initial={false}`
  - Change `animate={{ y: 0, opacity: 1 }}` to `animate={{ y: [0, -4, 0], scale: [1, 1.01, 1] }}` — subtle settle pulse
  - Keep existing `transition` delay/duration structure but adjust for the settle feel: `transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" }}`
  - NO `key` prop added
- `motion.div` (orange bar):
  - Change `initial={{ scaleX: 0 }}` to `initial={false}`
  - Change `animate={{ scaleX: 1 }}` to `animate={{ scaleX: [0.9, 1] }}` — bar extends slightly from left
  - Add `style={{ originX: 0 }}` if not already present (so it extends from left edge)
  - Keep spring-like transition: `transition={{ duration: 0.6, delay: 0.5, ease: "easeOut" }}`
  - NO `key` prop added
- `motion.p` (subtitle):
  - Change `initial={{ y: 50, opacity: 0 }}` to `initial={false}`
  - Change `animate={{ y: 0, opacity: 1 }}` to `animate={{ opacity: [0.8, 1], y: [6, 0] }}` — gentle rise and brighten
  - Keep existing transition timing: `transition={{ duration: 0.6, delay: 0.6, ease: "easeOut" }}`
  - NO `key` prop added
- `motion.div` (scroll indicator outer):
  - Change `initial={{ opacity: 0 }}` to `initial={false}`
  - Change `animate={{ opacity: 1 }}` to `animate={{ opacity: [0, 1] }}` — decorative element, fade-in is fine
  - Keep existing transition timing
  - NO `key` prop added
...
- Note: `useHydrated` is NOT needed in hero-section.tsx — the fix is unconditional `initial={false}` + settle keyframes

**about-section.tsx** (4 elements) — BELOW-FOLD, key remount:
...
**metrics-section.tsx** (1 element per metric, rendered in a map) — BELOW-FOLD, key remount:
...
**experience-timeline.tsx** (1 element per experience, rendered in a map) — BELOW-FOLD, key remount:
...
**tech-stack-section.tsx** (2 types of elements, rendered in nested maps) — BELOW-FOLD, key remount:
...
**graph-section.tsx** (1 element inside GraphSectionInner) — BELOW-FOLD, key remount:
...
2. Run `npm run build && npm run start`, then `curl http://localhost:3000` and verify the SSR HTML has no inline `style="opacity: 0"` on section content elements (hero heading, about paragraphs, metric cards, etc.)
3. Grep for `useHydrated` in all modified files to confirm import and usage
4. Grep for `initial={` across modified files -- every entrance-animation `initial` should reference `isHydrated`. Exceptions allowed: `motion.section` in hero (scroll-driven style prop, not entrance animation) and `motion.div` with `animate={{ y: [0, 10, 0] }}` (infinite bounce, no initial prop)
   </verify>
   <done>

- All 13 motion elements across 6 section components use hydration-aware `initial` prop
...
6. **Verify hero (above-fold):** The hero section (MANNAN text, orange bar, subtitle) should be visible immediately in the PPR shell AND remain visible through hydration with zero flash. After hydration, subtle settle animations should play: name text gets a tiny scale pulse, orange bar extends slightly from left, subtitle gently rises and brightens. Content should never disappear.
7. **Verify no flash:** Watch carefully during hydration -- below-fold sections should not flash. Hero should never disappear and reappear.
  </how-to-verify>
